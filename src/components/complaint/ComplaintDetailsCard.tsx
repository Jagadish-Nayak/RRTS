...
export function ComplaintDetailsCard({ 
  title, description, location, pincode, severity, status, submissionDate, estimatedExpense, rating , message , userName, phone, role
  ...
  <div className="flex flex-col md:flex-row md:justify-between">
    <div>
      ...
      <p className="text-sm text-gray-500">
        Submitted on <span className="font-medium">{submissionDate}</span>
      </p>
    </div>

    {['supervisor', 'admin', 'mayor'].includes(role) && (
      <div className="flex flex-col justify-end text-right mt-2 md:mt-0">
        {userName && (
          <p className="text-sm text-gray-500">
            <span className="font-medium">Submitted by:</span> {userName}
          </p>
        )}
        {phone && (
          <p className="text-sm text-gray-500">
